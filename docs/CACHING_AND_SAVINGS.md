...
1. How imgtokenx stays aligned with Anthropic prompt caching when it rewrites bulky context into images.
2. How imgtokenx reports savings without counting the provider cache discount as an imgtokenx win.
...
A stable prefix is cheap after it is cached, but a prefix that changes every turn repeatedly pays the write premium. imgtokenx's cache-alignment work exists to keep the image prefix stable.
...
imgtokenx rewrites only the bulky, cacheable parts into images. The dynamic parts stay as text so they do not pollute the image cache key.

The key invariant:

> imgtokenx does not add new cache-control markers. It relocates the caller's existing marker onto the last image block produced from the same logical content.
...
Switching modes can also burn a warm cache. If a text prefix is warm and imgtokenx flips to images, the first image turn may pay a cache write. If an image prefix is warm and imgtokenx flips back to text, the text path may pay the write. The symmetric burn terms model that cost:
...
The cache discount is provider behavior, not imgtokenx savings. To avoid counting cache as savings, both sides use the same observed cache state:
...
imgtokenx does not infer a warm text baseline from wall-clock TTL alone. The text request does not actually exist, so cache warmth for that counterfactual is based only on the real request's server-reported cache read.

For each `/v1/messages` request, imgtokenx records three measurements:
...
Assume a request whose original text baseline is `30,000` input tokens. Of those, `28,000` are before the cache-control marker, so `coldTail = 2,000`. imgtokenx renders that prefix to `3,000` image tokens.
...
Both sides are cold. imgtokenx is not credited for cache; it is credited because the cold image write is much smaller than the cold text write.

---

## Reproducing The Dashboard Math

Every row in `~/.imgtokenx/events.jsonl` carries the fields needed to reproduce the input-side savings:
