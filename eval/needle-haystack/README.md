...
(dense) traffic imgtokenx still saves ~68% as a *gist* tier — see FINDINGS.md correction.
...
| `crux.py` | 3 | feeds custom images straight to Opus (bypasses imgtokenx's renderer) to separate *encoder can't read hex* from *render too dense*. Billboard 120pt → 8/8; clean 30pt → 8/8. The encoder reads hex fine when it's big. |
...
1. Start the imgtokenx proxy on `127.0.0.1:47821`.
...
traffic imgtokenx measured ~68% fewer input tokens. Still worth re-running `sweep.py`
