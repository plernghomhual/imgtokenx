...
before rendering and targets only the configured Sol model.
...
**Policy decision:** Sol is text-only by default. Its renderer remains available,
but imaging requires explicit `IMGTOKENX_MODELS` scope plus an
`IMGTOKENX_READER_PROFILES` safety override. It should not become reader-safe by
...
`detail: "original"`. The harness rejects the known imgtokenx listener port 47821,
