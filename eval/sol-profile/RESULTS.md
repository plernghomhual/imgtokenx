...
Endpoint: direct OpenAI-compatible Responses path, bypassing imgtokenx

Image detail: **`original`**

This pilot tests raw reading of rendered terminal text. It does not include
imgtokenx's verbatim fact-sheet, and it does not compare Sol directly with Fable.
...
The 5×8 line is a real paid model call, not an inference from local rendering.
...
3. Production Responses transforms attach a verbatim fact-sheet for paths and
   identifiers. That defense may preserve covered exact strings in real imgtokenx
...
`gpt-5.6-sol` is **text-only by default** after these failures: silent image
rewriting requires positive reader evidence, not only positive token savings.
The render profile remains available for an explicit operator override:
