 ...
 * directly to OPENAI_BASE_URL / Responses and refuse imgtokenx's known local port,
...
const PROFILES = {
  ...
  retuned_sol_9x12: {
    ...
    // a larger effective cell density for the final retune candidate.
...
function responsesEndpoint() {
  ...
  // The eval must hit the direct upstream Responses endpoint, not imgtokenx.
  invariant(url.port !== '47821', 'Refusing imgtokenx port 47821: raw image eval must bypass imgtokenx');
...
const preflight = {
  ...
  tokenNotes: {
    image: 'imgtokenx production visionTokensForModel(gpt-5.6-sol, width, height) patch estimate',
