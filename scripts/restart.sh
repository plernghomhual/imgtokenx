...
# --- Parse our own flags out of "$@". --no-build only — imgtokenx takes none. ----
DO_BUILD=1
for arg in "$@"; do
  case "$arg" in
    --no-build)
    ...
    *)
      ...
      echo "[restart] this script only accepts --no-build (imgtokenx takes no flags)" >&2
      exit 2
      ;;
  esac
done
...
if [ -n "$PIDS_RAW" ]; then
  ...
  echo "[restart] found running imgtokenx proxy PID(s): $PIDS"
