...
## Final Review - 2026-07-10 (audit batch 18 — 1 of 38 items: #4 D4)

Status: 1 item implemented + regression-tested; tsc clean (0); vitest focused tests pass:
- tests/image-budget.test.ts 14/14 (countExistingImages + RequestImageCounter + transformRequest E2E)
- Surrounding test guardrails 7 files / 131 tests still green
Build green (0.8.0); `node scripts/release-check.mjs` -> "OK: ready to release v0.8.0"; git diff --check clean. Committed on `main` (no push per scope).

### Items completed (verified)
- [x] #4 D4 enforce one request-wide Anthropic 100-image budget. New `RequestImageCounter` class (single instance per `transformRequest`, threaded to every emission site) + `countExistingImages` helper. Counter is created after the reader-profile check; `total = applyAnthropicImageBudget ? o.maxImagesPerRequest : Infinity` (non-Claude models get a no-op). Slab: claim N images after render, slice to `slabImages`; inner loop skips emission for `i >= slabImages.length` (keeps cache anchor at the front, drops trailing pages — text-equivalent via the fact-sheet sidecar). Tool_result: clamp `effectivePerResult = min(maxImagesPerToolResult, remaining)`; on `remaining === 0`, push the original block as text (NOT an empty recovery ref). Telemetry: new `imageBudget?: { total, existing, used, skipped }` on `TransformInfo`, populated at the final return of `transformRequest`. New `maxImagesPerRequest` (default 100) and `applyAnthropicImageBudget` (default true) options on `TransformOptions`.

### Scope choices (deliberately deferred)
- History-collapse integration: NOT budgeted. History images naturally fit within the existing per-tool-result-style cap because the freezeChunk pipeline splits on a 10-message grid; a 50-message collapse produces ~5 history images, well under 10. Adding `maxImages` to `HistoryCollapseOptions` is a non-trivial design decision (per-chunk vs whole, what to do when budget tight) and is intentionally out of scope for this batch.
- `info.imageBudget` is only populated on the success path of `transformRequest`. Early returns (parse_error, reader_profile_unsafe, lossless_exact, below_min_chars, not_profitable) don't set it — those paths don't emit images, so the budget is untouched (all zeros).
