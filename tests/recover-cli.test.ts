...
describe('imgtokenx recover CLI', () => {
  it('prints the newest local recoverable source for a rec_* id', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'imgtokenx-recover-cli-'));
    try {
      fs.writeFileSync(
      ...
      const run = spawnSync(
        ...
        ['--import', 'tsx', 'src/node.ts', 'recover', 'rec_deadbeef'],
        {
          cwd: repoRoot,
          encoding: 'utf8',
          env: { ...process.env, IMGTOKENX_RECOVERABLE_DIR: dir },
          timeout: 30_000,
        },
      );
  ...
  it('defaults to ~/.imgtokenx/recovery when IMGTOKENX_RECOVERABLE_DIR is unset', () => {
    // Recovery is default-on: with no env var set, the CLI must resolve the
    // same ~/.imgtokenx/recovery default the live proxy writes sidecars to
    // (see resolveRecoverableDir() / defaultRecoverableDir() in src/recovery.ts).
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'imgtokenx-recover-home-'));
    try {
      const dir = path.join(home, '.imgtokenx', 'recovery');
  ...
  it('fails closed when IMGTOKENX_RECOVERABLE_DIR is explicitly disabled', () => {
    const env = { ...process.env, IMGTOKENX_RECOVERABLE_DIR: 'off' };
    const run = spawnSync(
      process.execPath,
      ['--import', 'tsx', 'src/node.ts', 'recover', 'rec_deadbeef'],
      { cwd: repoRoot, encoding: 'utf8', env, timeout: 30_000 },
    ...
    expect(run.stderr).toContain('IMGTOKENX_RECOVERABLE_DIR is disabled');
