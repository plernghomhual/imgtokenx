...
describe('reflow L0 contract – real corpus', () => {
  ...
  it('L0 contract holds for ~/.imgtokenx/4xx-bodies/ if present', () => {
    const dir4xx = resolve(homedir(), '.imgtokenx', '4xx-bodies');
    if (!existsSync(dir4xx)) {
      console.log('[4xx-bodies] ~/.imgtokenx/4xx-bodies not found — skipping');
      return;
    }
